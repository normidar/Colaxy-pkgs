...
#### 4-A. `colaxy_localization` の説明文が上限を超えうる (新規に発見した欠陥)

`LocaleUnit._getDescription()` は 4000 文字を検査するが、
`_fitDescriptionToFastlane()` は**その後で**
`\n\n[Minimum supported app version: X]` を追記する。
→ **上限ぎりぎりの説明文はディスク上で 4000 文字を超える。**
生成側では原理的に捕まらず、Play に送って初めて弾かれる。

iOS 側も同様 (`[:mav: X]` を追記)。

**直し方は `colaxy_localization` 側**: 追記後の長さで検査する。
`MetadataCheck` はこれを warning として拾うが、対症療法にすぎない。
...
| # | 内容 | 状態 |
|---|---|---|
| 9-1 | **`doctor` サブコマンド。** 認証、権限、必要ファイルの有無を検査する | ⚠️ **半分。** `--check` で**ファイルの検査は実装済み** (Stage 4)。**認証と権限の検査は未実装** — これは実アカウントが要るので U-8 と同時にやる |
