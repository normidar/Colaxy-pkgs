...
> **ステータス**: ✅ = 対応済み、⚠️ = 調査の結果 前提が変わった項目、無印 = 意図的に残した項目。
> 対応済みの内容は各パッケージの CHANGELOG.md にも記載しています。
> 残した項目とその理由は末尾の「10. 残タスク」にまとめています。
...
| ✅ 0-6 | **P2** | **テストがほぼプレースホルダー。** → 全 9 パッケージに実テストを追加（合計 71 件）: riverpod_helper 16 / app_theme_picker 9 / colaxy_adaptive_scaffold 9 / colaxy_screenshot 9 / colaxy_localization 8 / colaxy_icons_launcher 7 / colaxy_tutorial 6（新規）/ app_info_tile 5 / app_lang_selector 2。<br>原文:  `app_info_tile` / `app_lang_selector` / `app_theme_picker` / `colaxy_screenshot` / `riverpod_helper` の `test/main_test.dart` は中身が空の `// Implement the test`。`colaxy_localization` は `test('test1', () {})`。`colaxy_tutorial` に至っては `test/` ディレクトリ自体が無く、`melos run test` の `dirExists: test` フィルタで**黙ってスキップ**される。 |
...
| 1-10 | **P3** | 検索フィルタ（言語数が 38 あるので）、現在の言語のハイライト、RTL 言語の考慮。 → **未対応**: 新機能（P3）。 |
...
| 2-9 | **P3** | カスタムカラー（`FlexScheme.custom`）を選ぶ UI が無いのに `CustomThemeMock` だけ存在する。 → **未対応**: 新機能（P3）。 |
...
| 4-3 | **P2** | **アスペクト比でレイアウトを切り替える設計自体が Material 3 のブレークポイント指針に反する。** 縦長のタブレット（例: iPad 縦、比率 0.75）は幅 834px あるのに BottomNavigation になる。`LayoutBuilder` + 幅ベースのブレークポイント（600 / 840dp）を選択肢として追加すべき。 → **未対応**: レイアウト分岐そのものの設計変更。既存アプリの見た目が変わるため別 PR で。 |
...
| 5-4 | **P2** | **`SharedPreferences.getInstance()` が全メソッドで毎回呼ばれる**（`Prefs` の 7 メソッド + 各 Pod の 3 メソッド × 11 Pod ≒ 40 箇所）。キャッシュ済みインスタンスとはいえ、`SharedPreferencesAsync` / `SharedPreferencesWithCache`（shared_preferences 2.3+ の推奨 API）への移行も検討したい。`shared_preferences: ^2.0.0` という緩い制約も上げるべき。 → **未対応**: `SharedPreferencesAsync` への移行は保存フォーマットとタイミングが変わるため、別途検証が必要。 |
...
| 5-12 | **P3** | `Prefs` にキー一覧取得 / 全消去 / 型付きキー定義（`PrefKey<T>`）が無い。 → **未対応**: 新規 API の設計判断が必要（P3）。 |
...
| 6-15 | **P3** | `Future.delayed(3秒)` などの固定待機に依存。`WidgetsBinding.instance.endOfFrame` を使っているのだから、フレーム安定を検出する方式にできると高速化できる。 → **未対応**: 実機での撮影タイミングに関わるため、実環境での検証が必要（P3）。 |
...
| 7-4 | **P2** | **`TutorialTool.tutorialVisible` がグローバルな static mutable。** テストや複数画面での制御が難しい。Riverpod provider 化（このリポジトリの他パッケージは全部 Riverpod）に揃える。 → **未対応**: グローバルスイッチとしての利便性とのトレードオフ。Riverpod 化は破壊的変更になるため方針判断が必要。 |
...
| 7-10 | **P2** | `SharedPreferences` 直叩き。同 monorepo の `riverpod_helper` に `Prefs` があるのに使っていない（`shared_preferences` を直接依存）。 → **未対応**: `riverpod_helper` への依存追加になるため、パッケージ間の結合方針の判断が必要。 |
...
| 9-4 | **P2** | **`exit(1)` による即時終了がライブラリコード内に散在**（`cli_commands.dart`）。ライブラリとして呼ぶと呼び出し元のプロセスごと落ちる。例外を投げて `bin/` 側で `exit` する構造にする。 → **未対応**: fork 元由来の構造。`exit(1)` は 2,949 行に散在しており、9-6 とまとめて扱うべき。 |
| 9-5 | **P2** | **`late _FlavorHelper _flavorHelper;` というトップレベル可変グローバル**（`cli_commands.dart:20`）。複数回呼ぶと状態が漏れる、テスト不能。 → **未対応**: 9-4 / 9-6 と同じリファクタの一部。 |
| 9-6 | **P2** | **`part` による 2,949 行の巨大な単一ライブラリ。** `src/android.dart`（580行）等が `part of` なので単体テスト不可。 → **未対応**: `part` ベースの巨大ライブラリの分割は fork 元との差分が大きくなり、上流追従が困難になる。 |
...
| 9-8 | **P2** | **他パッケージと lint 設定が別系統**（`lints/recommended` vs `very_good_analysis`）。fork 由来だが、monorepo として統一するか、意図的な例外である旨を書く。 → **未対応**: lint 基盤を `very_good_analysis` に揃えると fork 元コードに大量の指摘が出る。上流追従とのトレードオフ。 |
...
ほぼすべての項目が対応済みです（上の表の ✅）。意図的に残しているものは以下の 4 件のみで、
いずれも「こちらの一存で決めるべきではない」または「この環境で検証できない」ものです。

| # | 内容 | 残した理由 |
|---|------|-----------|
| 0-3 | `riverpod_lint` の有効化 | Dart 3.12.2 では `plugins:` 宣言により `dart analyze` がハングして終了しない（プラグインなし 1.3 秒 / あり 6 分でも未終了）。CI を無限に止めるリスクがあるため未有効化。手順はルートの `analysis_options.yaml` にコメントで記載済み |
| 4-3 | アスペクト比 → 幅ブレークポイント方式 | `AdaptiveScaffold` の分岐条件そのものを変える設計変更。既存アプリの見た目が変わるため、方針を決めてから別 PR で |
| 5-4 | `SharedPreferencesAsync` への移行 | 保存フォーマットとタイミングが変わる。移行パスの検証が必要 |
| 7-4 | `tutorialVisible` の Riverpod 化 | グローバルスイッチとしての手軽さとのトレードオフ。破壊的変更になる |
| 7-10 | `colaxy_tutorial` から `riverpod_helper.Prefs` を使う | パッケージ間の結合を増やす判断が必要 |
| 9-4 / 9-5 / 9-6 / 9-8 | `colaxy_icons_launcher` 内部構造の整理 | fork 元由来の 2,949 行・`part` ベース構造。手を入れると上流追従が困難になるため、fork 方針とセットで判断すべき |
| 1-10 / 2-9 / 5-12 / 6-15 | 検索フィルタ・カスタムカラー UI・型付きキー・撮影待機の最適化 | いずれも改善ではなく新機能（P3）。要否の判断が必要 |

### 部分的に残っているもの

- **5-5**: `prefs_riverpod` / `prefs_alive_riverpod` の重複そのものは残っています。`riverpod_generator` はジェネリックな Notifier を生成できないため、共通基底クラスへの集約は codegen の制約に阻まれます。非対称だった `PrefsAliveMapPod` は追加済みなので、実害（API の欠落）は解消しています
- **8-14**: `LocaleApp` / `LocaleUnit` / `AndroidNameLocalization` / `IOSNameLocalization` はすべて `rootPath` を受け取るようになりました。`fastlane/metadata` などのサブパス自体はまだ固定です
