...
### Stage 4 — 中間形式の型検証 (Android で完結した直後にやる)

| # | 内容 |
|---|---|
| 4-1 | **`colaxy_localization` / `colaxy_screenshot` の出力を、投入前に型で検証する。** 現在は fastlane が読むまで誤りが分からない |
| 4-2 | ロケール名の不一致、必須ファイルの欠落、文字数超過を投入前に検出する |
| 4-3 | ただし**ストアが判定すべきものはストアに任せる** (2-3 の原則)。検証するのは「規約上明らかに壊れているもの」だけ |

**完了条件**: 生成 → 検証 → 投入がローカルで完結し、ストアに送る前に構造的な誤りが出る。

> これは fastlane では原理的にできない。生成側と投入側が同じ言語だから可能になる利点で、
> **「Ruby を消す」以外の実質的な価値**がここで初めて出る。
...
| # | 内容 |
|---|---|
| 9-1 | **`doctor` サブコマンド。** 認証、権限、必要ファイルの有無を検査する。`colaxy_store_console` の `verify` と同じ性格で、セットアップの失敗経路が多いので価値がある |
| 9-2 | README と例。段階ごとに「fastlane の何を置き換えるか」を対応表で書く |
| 9-3 | **fastlane と併用できることを明記する。** 全部を一度に置き換える必要はなく、`supply` だけ差し替える使い方ができるべき。これが移行を現実的にする |
